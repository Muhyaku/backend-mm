...
// 4. SCHEMA SETTINGS DINAMIS (BARU)
...
const initSettings = async () => {
  ...
  if (count === 0) {
    await Setting.insertMany([
      { settingKey: 'scramble_keypad', name: 'Acak Tombol PIN Kasir', description: 'Mengacak posisi angka di layar login untuk mencegah orang menghapal gerakan jari kasir (Anti-Ngintip).', isActive: false },
      { settingKey: 'pin_table_column', name: 'Bekukan Kolom Tabel (Pin Header)', description: 'Membekukan kolom Aksi dan Total di laporan agar tidak ikut tergeser saat di-scroll menyamping (Cocok untuk Layar Tablet).', isActive: true }
    ]);
  }
};
initSettings();
